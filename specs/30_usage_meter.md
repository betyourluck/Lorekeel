...
**単価とコンテキスト長のフォームを消さない規則** (実機 2 件、failures #103 と 2026-09-14): 保存後の
選択同期は純関数 `selectionAfterSave` (`aiProfiles.ts`) で、フォームを差し替えるのは**別の登録に
一致したときだけ** (同じ登録のまま = #103 / どの登録にも一致しない = 09-14、思考の深さや出力上限を
変えると登録と .env の一致が外れる)。「保存 + 登録モデルを更新」は単価を .env を書く前に取り出し、
同期そのものを飛ばす (vitest +4)。
