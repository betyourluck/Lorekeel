...
- エピローグは**会話ログにのみ**積む。chronicle には積まない（終幕なので次ターンは無いが、
  ...
  **←2026-09-19 改訂（ユーザー要望「エピローグがなんであったか楽しめたほうが面白い」）**:
  `SessionSave.epilogue: Option<String>`（serde default）に**生成済みの本文だけ**を保存する。
  終幕の後に再開・スロットのロードをすると、結末までの語りは出るのにエピローグだけが消えていた。
  当初の「入れない」の理由は**生成の失敗・クラッシュがセーブを巻き込まない**ことで、これは保つ —
  受理ターンの autosave を先に書き（従来どおり）、**生成に成功した後にもう一度** autosave する。
  失敗なら 1 回目のセーブのまま（エピローグ欄は空）。chronicle に積まない・GM に還流しない、は不変。
  再開時は `ResumeView.epilogue` で「前回までの語り」の後にマーカーつきで出す（読み上げはしない）。
  CLI は終幕のターンで `break` するので終幕後のセーブ自体を書かない（GUI だけの機能）。
