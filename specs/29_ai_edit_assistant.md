...
Phase E（3 モデル × 3 依頼の実測）は未着手。**目視で出た不具合 2 件を同日回収**: ①パネルを
`v-if`/`v-else` の対の間に挟んで `v-else` が付け替わり、閉じると「ファイルを選んでください」が
本文の下に出た ②spinner が回らず「固まっている」ように見えた（`Icon` の spinner は静的な弧で
回転は使う側が `animate-spin` を付ける約束、付け忘れ。保存ボタンも同じ抜け）。あわせて道具の
呼び出しの合間にも動きが出るよう、進行ログ末尾に「応答を待っています」の脈動行を足した。
**追補（同日、ユーザー要望）**: 右ペインの検査（層 2）の見出しに「AI に直させる」— **開いている
ファイル宛て + パッケージ全体宛て**の指摘を指示欄へ流してパネルを開く（他ファイル宛ては、その
ファイルを開いてから = AI 編集が 1 ファイルを対象にする線に揃える）。打ちかけの指示があれば末尾へ
足す。指示文には「原因が他ファイルにあるなら読んで確かめ、このファイル側でできる直し方を選べ」を
添える（読めるが書けない、という道具の境界を先に伝える）。
