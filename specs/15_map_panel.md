...
- **自前のスクロール枠を持たない**（2026-09-10、ユーザーが実機で「マップだけ位置が違う」と
  気づいた）。rev2 の初版は root に `overflow-y-auto` + `h-full` を置いており、右ペインの
  `p-4` の**内側にもう一枚スクローラができて**バーが他のタブより内側に出ていた。rev1 は掴んで
  パンするために高さを持つ必要があったが、リストにはない。中身をそのまま流し、スクロールは
  右ペインに任せる（凡例の `mt-auto` も外し、内容の後ろへ）。
- **右ペインのスクロールバーは線**（4px・`.scroll-hairline`、2026-09-10 ユーザー決定
  「このくらいの短さでスクロールバーを掴む人はいない。たぶんホイールを使う」）。
  **会話ペインは 8px のまま**（ユーザー指示 — あちらは長く、掴んで動かす人がいる）。
  つまみは `ash` から `parchment/30` へ替えた: ash はライトで地との比 1.39 しかなく、
  8px では見えていたものが 4px では消える（実測）。parchment は明暗で役割が入れ替わる
  唯一のトークンなので両テーマで線が残る（2.29 / 1.85）。
