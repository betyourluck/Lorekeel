...
4. **圧縮は受理ターン直後に同期実行**（rev2 査読確定。頻度 10〜20 ターンに 1 回・体感数秒。
   ...
   **2026-09-01 から設定「AIモデル」タブの select で選べる**）、
...
| 要約タイムアウト | 60 秒（既定・`SUMMARY_LLM_TIMEOUT_SECS` で上書き。2026-08-26 に 15 から。**2026-09-01 から GUI の select（60/90/120/180/240/300/600）**・`set_summary_timeout` はプロファイル選択と別 command＝互いを消さない・0 は空＝この既定へ） |
| 待ちの代償 | 同期実行なので**上限を上げるほど成功時の待ちも伸びる**。UI 文言で明示（この秒数だけ次の語りが待たされうる／失敗時は毎回その時間を払う／3 回で機械 join）。600 秒を選べるようになったことで、非同期化の見送り（rev2）の代償が可視化された |
