...
**Status**: rev2（2026-09-18 起草 → 同日査読 2 本を反映。決定 5 点はユーザー承認済み）→ **✅Phase 0（2026-09-19）** → **✅Phase A（同日）** → **✅Phase B（同日）** → **✅Phase C（同日、規律を凍結・未決 2 件を決着）= Done**。
以後は Phase C の規律で運用する。
査読の反映は末尾「査読の反映」節に凍結。
...
Phase A・B で実際に通った手順をそのまま規則にする。

1. **判定を先に書く**。直す不具合が「届く範囲」表のどの行に当たるかを決める。どの行にも当たらない
   新しい種類なら、**表に行を足してから**進む（届くか・理由）。表が現実の射程とずれていくのを防ぐ。
2. **届くなら `*.mount.test.ts` を 1 本付ける**。ファイル名で happy-dom の project に入る（決定 2）。
   - 冒頭の doc comment に「何が壊れていたか」と「**Red の確かめ方**（どの行をどう戻すと落ちるか）」を書く。
     後の人が修正を疑ったとき、同じ手順で Red を再現できるようにする。
   - 偽装の表はテストの冒頭に全部並べる。漏れは後始末の検出器が落とす（Phase B）ので、
     「ローダーが握り潰しているから動いた」は起きない。
   - **修正を一時的に戻して Red を確かめてから** Green にする（掟の PoC 必須の提示層版）。
     Red の出方が実機の症状と一致するかを見る — 一致しなければテストは別の物を測っている。
3. **半分だけ届くなら、届いた半分と届かない半分を doc comment と failures.md の両方に書く**
   （09-14 ダブルクリックの形。「テストがある」が「全部塞いだ」と読まれないように）。
4. **届かないなら、failures.md のその項目に「マウントテストの射程外（理由）」と 1 行書く**。
   理由は表の行をそのまま引く。接地の限界の申告であって、免責ではない（2026-08-28 の方法論節）。
5. **実寸を読む部品**（未決 1 の決着）: 実寸から先の計算を純関数へ出し、そちらを unit で固定する。
   マウントテストで実寸を偽装しない — happy-dom は全部 0 を返すので、偽装した数値は実機の数値と
   別物になり、通っても何も言えない。`FirstRunTour` の `placeCard` / `cardWidthFromCss` が先例
   （2026-09-07 に実機で切れた形そのものを unit の Red にした）。
6. **足した回に数を直す**。CLAUDE.md の frontend の行（unit / mount の内訳つき）を同じコミットで更新する。

**機械の網は置かない**（考えて見送った）: 「failures.md の app/src 項目は全部テストか射程外の申告を
持つ」をテストで強制する案は、failures.md が自由記述の台帳である以上、文字列一致に頼ることになり
壊れやすい。守りはこの節と CLAUDE.md の掟の 1 行（修正のたびに読まれる場所）に置く。
...
1. ~~`FirstRunTour` のようにレイアウトの実寸（`getBoundingClientRect`）を読む部品は、happy-dom では
   ...
   足りるとするか。~~ **←2026-09-19 決着: 純関数で足りるとする**（規律 5）。実寸を偽装したマウントテストは
   実機と別の数値を測るだけなので書かない。
2. ~~所要時間の上限。Phase 0 で `unit` / `mount` それぞれの時間を測り、`mount` が大きく伸びるなら
   `npm test` を `unit` だけにして `mount` を別スクリプトにするかを判断する（決定 2 の projects なら
   設定 1 行で切り替えられる）。~~ **←2026-09-19 決着: 分けない**。Phase B 後の実測（各 3 回）で
   `unit` 115 本 = 1.65〜2.27 秒、`mount` 19 本 = 1.91〜2.63 秒、`npm test` で両方 = 3.15 秒。
   mount は環境の準備（happy-dom 2 秒前後）が固定費で、本数に比例して伸びる部分は 1 本あたり
   数十ミリ秒。片方だけ回したいときは `npx vitest run --project unit` で足りるので、script も足さない。
   再判断の目安は `npm test` が 10 秒を超えたとき。
