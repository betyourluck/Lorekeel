...
4. **プロバイダ別の運び方**（encode 純関数）:
   ...
   - **ComfyUI**: 各参照を `POST /upload/image`（multipart `image`、`overwrite=true`）→ 返る `name` を
     ...
     画集があるのにワークフローに `%ref_1%` が無いときは警告を出す。**←2026-08-26 改訂**: 同梱していた
     参照つきテンプレート `comfy_krea2_ref.json`（Krea 2 Turbo + `TextEncodeQwenImageEditPlus`）は
     **撤去した**（ユーザー決定）。ComfyUI を用意できる人は自分の絵柄とモデルを持っており、SD 系や
     Z1 を使う人にはそのまま使えない一方、同梱すると**モデルの入手までサポート対象に見える**。
     残すのは汎用テンプレート 1 本（`comfy_generic.json`、参照なし）で、参照を使う人は `%ref_1%` を
     自分のワークフローへ差す。置換語そのものの説明はワークフロー欄のプレースホルダ文が持ち、
     入れ忘れは警告 3 本（`%ref_1%` / `%seed%` / `%negative%`）が拾う。フィクスチャとしては
     `app/src-tauri/src/testdata/comfy_ref_workflow.json` に残っており、剪定の live テストが使う。
