...
**Status**: Draft rev2（2026-08-20 起草 → 同日ユーザー査読 4 点 + 細部 3 点を反映。rev1 の
...
1. OpenAI `input_fidelity: high` を既定にするか — v1 は送らず、実測後に決める（決定 3 と整合）。
2. パッケージ同梱の設定画集を**書庫の審査**で見るか（outcast 側。画像は今も審査外）。
3. outcast `package_spec.md` への節の書き方（「設定画集の作り方」= 立ち絵に名前を書く・背景は分割して
   1 枚に）。
