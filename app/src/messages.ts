  ...
  ja: {
    ...
    settings: {
      ...
      image: {
        ...
        workflowPlaceholder: "ComfyUI で「Save (API Format)」したワークフローを貼り付け。%prompt% / %negative% / %seed% / %width% / %height% が置換されます。",
        insertGeneric: "汎用ワークフローを挿入",
        workflowNote: "挿入後、ckpt_name を自分のモデル名に書き換えてください。ComfyUI で一度開いて確認するのが確実です。",
        timeout: "タイムアウト（秒）",
        ...
        sheetsIntro:
          "パッケージの images/settings_sheets/ に置いた画像（png/jpg/webp、4MB 以下）をファイル名順に最大 3 枚、生成の参照として添付します。1 枚に複数人の立ち絵と名前、背景をまとめた「設定資料集」の形で作ると、挿絵の見た目がそれに寄ります。",
        sheetsNoGame: "ゲームを開始すると、そのパッケージの設定画集がここに出ます。",
        sheetsRefresh: "再読み込み",
        sheetsNone: "見つかりません（{dir}）",
        sheetsOversize: "4MB を超えています（スキップ）",
        sheetsUnsupported: "対応していない形式です（png/jpg/webp のみ）",
        sheetsOverLimit: "上限枚数を超えています（ファイル名順で先頭 3 枚まで）",
        folderHeading: "保存",
        folder: "保存フォルダ（空なら既定）",
  ...
  en: {
    ...
    settings: {
      ...
      image: {
        ...
        workflowPlaceholder: "Paste a workflow exported with ComfyUI's 'Save (API Format)'. %prompt% / %negative% / %seed% / %width% / %height% are substituted.",
        insertGeneric: "Insert generic workflow",
        workflowNote: "After inserting, replace ckpt_name with your own model. Opening it once in ComfyUI is the safest check.",
        ...
        sheetsIntro:
          "Images placed in the package's images/settings_sheets/ (png/jpg/webp, up to 4MB) are attached as references, up to 3 in filename order. Make them like a character sheet — several characters with their names written, backgrounds collected — and illustrations will follow their look.",
        sheetsNoGame: "Start a game and the sheets of that package appear here.",
        sheetsRefresh: "Reload",
        sheetsNone: "None found ({dir})",
        sheetsOversize: "Over 4MB (skipped)",
        sheetsUnsupported: "Unsupported format (png/jpg/webp only)",
        sheetsOverLimit: "Over the limit (first 3 by filename)",
