  ...
  ja: {
    ...
    settings: {
      ...
      display: {
        ...
        fontLarge: "大 (20px)",
        fontXlarge: "特大 (24px)",
        fontNote: "UI 全体の基準フォントサイズを変えます（即時適用・localStorage に保存）。会話ログの本文は、下の「本文の文字サイズ」が「UI に合わせる」のときだけ連動します。",
        fonts: {
          default: "標準 (UI と同じ)",
          mincho: "明朝",
          gothic: "ゴシック",
    ...
    refStock: {
      ...
      loading: "参照画像を読み込んでいます……",
      loadFailed: "参照画像を読み込めませんでした（理由は右下の通知に出ています）",
    ...
    packages: {
      ...
      add: "追加",
      newPkg: "新規作成",
      newPkgTitle: "置き場・パッケージ名・フォルダ名を決めて、空のパッケージ (フォルダ + package.yaml + 最小のシナリオ) を作る",
      newPkgHint:
        "置き場にフォルダを作り、package.yaml と最小のシナリオ (scenarios/main.yaml) を入れて一覧に登録し、そのまま編集モードで開きます。パッケージ名は一覧に出る題名 (日本語可)、フォルダ名はディスク上の名前です。",
      newPkgParent: "置き場のフォルダ (この中に作ります)",
      newPkgPickParent: "置き場をフォルダ選択ダイアログで選ぶ",
      newPkgTitleInput: "パッケージ名 (一覧に出る題名・日本語可)",
    ...
    editAssist: {
      heading: "AI に直させる",
      toggleTitle: "AI に直させる — 開いているファイルだけを、指示に従って書き換えます",
      hint: "対象は開いているこのファイルだけ。他のファイルは読めますが書き換えません (新しいファイルも作りません)。置換のたびに検査が走り、結果は未保存の本文として入ります (Ctrl+Z で戻せます)。目安の順: package.yaml → characters → scenarios。",
      placeholder: "例: 主人公の HP を 12 にして / SAN 判定の challenge を 1 つ足して / ヒロインの profile に趣味を足して",
      run: "実行",
      running: "直しています…",
      cancel: "取り消し",
      ...
      fileSwitched: "実行中にファイルを切り替えたので、{file} への結果は捨てました",
    ...
    editor: {
      ...
      mediaNote:
        "ファイル名がそのままアセット ID です — YAML の image / bgm / sound / icon 欄に書きます (エディタの補完に出ます)。クリックでプレビュー、ダブルクリックで名前を変更 (参照している YAML も書き換わります)。画像は WebP へ変換して取り込みます (音声は原本のまま — Ogg 推奨)。",
      ...
      assetRenameTitle:
        "名前を変えると、パッケージ内の YAML でこの名前を指す欄 (画像は image / icon、音声は bgm / sound) も書き換わります",
      assetRenameNeedsSave: "未保存の変更があります — 保存してからアセットの名前を変えてください (参照している YAML を書き換えるため)",
      assetRefsRewritten: "参照を書き換えました ({n} ファイル): {files}",
      assetRefsRewriteFailed: "名前は変わりましたが、参照の書き換えに失敗しました: {error}",
      newTitle_character:
  ...
  en: {
    ...
    settings: {
      ...
      display: {
        ...
        fontNote: "Changes the base font size of the whole UI (applied instantly, saved to localStorage). The conversation-log body follows it only while \"Body text size\" below is set to \"Match the UI\".",
        fonts: {
        ...
        msgSize: "Body text size",
        msgSizeFollow: "Match the UI",
        font: "Font",
        color: "Text color",
    ...
    refStock: {
      ...
      loading: "Loading reference images…",
      loadFailed: "Could not load the reference images (see the notice at the bottom right).",
    ...
    packages: {
      ...
      add: "Add",
      newPkg: "New package",
      newPkgTitle: "Pick a location, a package name and a folder name to create an empty package (folder + package.yaml + a minimal scenario)",
      newPkgHint:
        "Creates a folder inside the location, writes package.yaml and a minimal scenario (scenarios/main.yaml), registers it in the list and opens it in edit mode. The package name is the title shown in the list; the folder name is the name on disk.",
      newPkgParent: "Location (the folder is created inside this)",
      newPkgPickParent: "Choose the location with a folder picker",
      newPkgTitleInput: "Package name (the title shown in the list)",
    ...
    editAssist: {
      heading: "Let AI edit",
      toggleTitle: "Let AI edit — rewrites only the open file, following your instruction",
      hint: "Only the open file is edited. Other files can be read but are never changed (and no new files are created). Every replacement is linted, and the result lands as unsaved text (Ctrl+Z reverts). Suggested order: package.yaml → characters → scenarios.",
      placeholder: "e.g. set the hero's HP to 12 / add one SAN check challenge / add a hobby to the heroine's profile",
      run: "Run",
      running: "Editing…",
      cancel: "Cancel",
      ...
      fileSwitched: "You switched files while it ran, so the result for {file} was discarded",
    ...
    editor: {
      ...
      mediaNote:
        "The file name is the asset id — write it in a YAML image / bgm / sound / icon field (it shows up in the editor's completion). Click to preview, double-click to rename (YAML that references it is updated too). Images are converted to WebP on import (audio is kept as-is — Ogg recommended).",
      ...
      assetRenameTitle:
        "Renaming also updates the YAML fields in this package that point at this name (image / icon for images, bgm / sound for audio)",
      assetRenameNeedsSave: "You have unsaved changes — save before renaming an asset (its references in YAML get rewritten)",
      assetRefsRewritten: "Updated references ({n} files): {files}",
      assetRefsRewriteFailed: "Renamed, but updating the references failed: {error}",
      newTitle_character:
