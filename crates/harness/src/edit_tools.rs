//! AI 編集 (spec 29 Phase C) — 道具 5 本の**実体** (**2026-09-06 に app から harness へ移設** — `play edit` が同じ実体を使う。app は pub use の包み) (`read` / `grep` / `sd` / `diff` / `spec`) と、
//! 語彙から system の表を描く純関数。ループそのものは [`crate::edit_assist`] (純関数 +
//! fake で固定) で、ここは編集ルートの中だけを触る [`crate::edit_assist::ToolExecutor`]。
//!
//! 境界 (spec 29 決定 1〜6):
...
use crate::edit_assist::{Diag, ToolExecutor, ToolReply};
...
pub struct EditSession {
    ...
    initial: String,
    buffer: String,
    /// 成功した preview の (pattern, replacement, case_insensitive) の集合。apply の前提で、
    /// apply が消費する。**集合**なのは、独立した置換を 1 周でまとめて preview し次の周で
    /// まとめて apply できるようにするため (直前 1 件だけだと 2 本目の preview が 1 本目を消し、
    /// 周回が置換ごとに 2 周ずつ要る = 上限に当たる原因だった、2026-09-06 実測)。
    previewed: std::collections::BTreeSet<(String, String, bool)>,
}
...
impl EditSession {
    pub fn new(root: &Path, target_rel: &str, kind: &str, files: Vec<String>, initial_text: &str) -> Self {
        Self {
            ...
            previewed: std::collections::BTreeSet::new(),
        }
    }
...
")
    ...
    fn tool_sd(&mut self, args: &Value) -> ToolReply {
        ...
        if apply && !self.previewed.contains(&key) {
            return ToolReply {
        ...
        if apply {
            ...
            // 本文が変わったので、他の preview の差分ももう正しくない = 全部消す。
    ...
    fn tool_spec(&self, args: &Value) -> ToolReply {
        ...
        let idx = crate::package_spec::index();
        match idx.topics.iter().find(|t| t.id == topic) {
            Some(t) => match crate::package_spec::slice(&t.slice) {
                Some(text) => ToolReply { body: crate::package_spec::expand(text), ok: true },
...
/// unified diff (道具の `sd` / `diff` と `play edit` の表示が同じ形)。
pub fn unified_diff(display: &str, before: &str, after: &str) -> String {
...
";
    ...
    fn sd_requires_a_matching_preview_before_apply_and_reports_counts() {
        ...
        // apply 後は preview の記憶が全部消える (本文が変わった = 他の差分も古い)。
        assert!(!s.call("sd", &json!({"pattern": "(?m)^title: U$", "replacement": "title: W", "apply": true})).ok);
        // 独立した置換をまとめて preview → まとめて apply (1 本目の apply で 2 本目の記憶は消える
        // ので、2 本目は preview からやり直し = 差分が古くならない)。
        assert!(s.call("sd", &json!({"pattern": "entry: ", "replacement": "entry:  "})).ok);
        assert!(s.call("sd", &json!({"pattern": "title: U", "replacement": "title: V"})).ok);
        assert!(s.call("sd", &json!({"pattern": "title: U", "replacement": "title: V", "apply": true})).ok);
        assert!(!s.call("sd", &json!({"pattern": "entry: ", "replacement": "entry:  ", "apply": true})).ok, "apply 後の古い preview は無効");
        // 0 件・無変更・path 指定。
        ...
        assert!(d.body.contains("-title: T") && d.body.contains("+title: V"), "{}", d.body);
        std::fs::remove_dir_all(&dir).ok();
    }
