//! 型のそばの doc comment を機械抽出する (spec 28 Phase C-2、2026-08-28。**2026-09-06 に app の
//! `editor_docs` から harness へ移設** — spec 29 Phase B の `play spec-vocab` (CLI) が同じ表を
//! 使うため。app 側は `pub use` の薄い包み)。
//!
...
const SOURCES: &[&str] = &[
    include_str!("../../gm_core/src/spine.rs"),
    include_str!("../../gm_core/src/state.rs"),
    include_str!("package.rs"),
    include_str!("campaign.rs"),
    include_str!("memoria.rs"),
];
...
fn tidy(doc: &str) -> String {
    ...
    // `[`StateOp::RecordTurn`](crate::StateOp::RecordTurn)` の宛先側 `(crate::...)` を剥がす
    // (上で表示名だけ残した後に宛先が裸で続く)。
    while let Some(a) = s.find("(crate::") {
        let Some(rel) = s[a..].find(')') else { break };
        s.replace_range(a..a + rel + 1, "");
    }
