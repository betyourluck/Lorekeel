...
//!
//! **説明 (2026-08-28)**: 候補には doc comment 由来の説明が付く ([`crate::docs`])。
...
fn build_key_tables() -> EditorVocabulary {
    ...
    for (name, info) in gm_core::context_keys() {
        ...
        let items = info
            ...
            .iter()
            .map(|k| {
                let doc = ty.as_deref().and_then(|t| docs::field_doc(t, k));
                VocabItem::with(k, doc)
            })
            .collect();
    ...
    // harness 側の文脈。
    v.contexts.insert(CTX_MANIFEST.into(), keys_of::<crate::PackageManifest>("entry: x"));
    v.contexts.insert(CTX_PLAYER.into(), keys_of::<crate::PlayerDef>("{}"));
    v.contexts.insert(CTX_GLOBALS.into(), keys_of::<crate::Globals>("{}"));
    v.contexts.insert(
        CTX_CAMPAIGN.into(),
        keys_of::<crate::Campaign>("start: a
modules: { a: x.yaml }
edges: []"),
    );
    v.contexts.insert(
        CTX_CAMPAIGN_EDGE.into(),
        keys_of::<crate::CampaignEdge>("from: a
on_goal: g
to: b"),
    );
    v.contexts.insert(CTX_MEMORIA.into(), keys_of::<crate::MemoryFragment>("text: x"));
    ...
    v.tag_values.insert(
        ...
        gates
            .keys()
            .map(|k| VocabItem::with(k, docs::variant_doc("Gate", k)))
            .collect(),
    );
    v.tag_values.insert(
        "op".into(),
        ops.keys()
            .map(|k| VocabItem::with(k, docs::variant_doc("StateOp", k)))
            .collect(),
    );
...
fn variant_items(
    ...
    table
        ...
        .map(|(k, fields)| {
            let items = fields
                .into_iter()
                .map(|f| VocabItem::with(&f, docs::variant_field_doc(ty, &k, &f)))
                .collect();
            (k, items)
        })
        .collect()
...
fn collect_ids(root: &Path) -> BTreeMap<String, Vec<VocabItem>> {
    ...
    if let Ok(text) = std::fs::read_to_string(root.join("package.yaml")) {
        if let Ok(m) = serde_yaml::from_str::<crate::PackageManifest>(&text) {
    ...
    if let Ok(text) = std::fs::read_to_string(root.join("campaign.yaml")) {
        if let Ok(c) = crate::Campaign::from_yaml(&text) {
...
mod tests {
    ...
    fn vocabulary_fixture_is_fresh() {
        let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../app/src/editorVocabulary.fixture.json");
...
";
    ...
    fn ids_survive_a_half_written_package() {
        ...
        assert!(crate::load_package(&dir).is_err(), "前提: この盤面は validate で落ちる");
