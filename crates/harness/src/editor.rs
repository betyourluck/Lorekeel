...
//! `crate::inspect` の既存 5 系統を呼ぶだけにする)。
//!
...
/// アセット ID の stem を安全化する (`crate::is_valid_asset_id` の charset に合わせる)。
...
pub fn replace_media(root: &Path, rel: &str, bytes: &[u8]) -> Result<(), String> {
    ...
    if !MEDIA_DIRS.iter().any(|(d, _)| *d == dir) || !crate::is_valid_asset_id(name) {
...
pub fn rename_file(root: &Path, rel: &str, new_name: &str) -> Result<String, String> {
    ...
    if is_media {
        if !crate::is_valid_asset_id(new_name) {
            return Err(format!(
...
pub fn delete_file(
    ...
    let path = if let Some((dir, name)) = rel.split_once('/') {
        if MEDIA_DIRS.iter().any(|(d, _)| *d == dir) {
            if !crate::is_valid_asset_id(name) {
                return Err(format!("不正なアセットです: {rel}"));
            }
