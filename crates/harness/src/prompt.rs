...
無害 (何も起きない) なので、所持が不確かでも拾ってから使ってよい。\
**add_item / remove_item は主人公の手元にしか効かない。** NPC が持っている物を使う・消費する\
なら、先に give_item (from: その NPC, to: player) で主人公の手に渡してから remove_item を\
並べよ (NPC の所持品に直接 remove_item は通らない)。逆に NPC に拾わせるなら add_item で\
主人公が拾い give_item で渡す。ただし\
