...
【←2026-08-31 追記】`use_tts` (作者宣言ゲート) 自体を撤去し、読み上げは設定サウンドタブの
チェックボックス一本 (既定 OFF・imageGen.enabled と同型) になった。本エントリの前提
(「宣言された盤面でだけ操作が出る」) は消滅したが、一般化は生きている — 新形では
「設定で有効化」が宣言に当たり、その下流のクイックトグルは既定 ON (二重に隠さない)。
...
**【2026-08-21 追記 = 粒度の訂正】** ユーザー実プレイで input 12206 → **cached 10496** (86%)。
10496 は 8192 の倍数ではない (= 8192 + 2304 = 128×82) ので、「8192 ブロック単位」は
合成パディングで倍数ばかり踏んだ偶然だった。**正しくは「床 8192・粒度はそれより細かい
(128 の倍数と整合 = xAI #45 と同じ粒度の可能性)」**。床 (7.7k→0) の主張は不変で、
Kataribe の cache_floor=8192 もそのまま正しい。
...
**【同日追記 = ①の訂正】「Responses 口は確実」は n=2 の過剰一般化だった** (#53/#57 の方法論、
そして 2026-08-09 の Meta 互換口の訂正と**全く同じ轍を同じプロバイダで踏んだ**)。追加 probe
15 リクエスト: **実プレイの形 (固定プレフィックス + 可変 user) は 0/9**・完全同一の再送でも
2/4。最初の「1107 中 1073」「live 3889」は**どちらも同一リクエストの再送がたまたま当たった**
もので、GM ターン (毎ターン user が変わる) では構造的に効かない。ユーザーの実プレイ (turn 6
で警告発火) が正しい観測だった — **警告は嘘をついておらず、私の検証が実戦の形をしていなかった**。
処方を訂正: Meta の Responses 化は維持 (400 降格ダンスの解消・effort・usage 詳細は実利)、
キャッシュは「不安定」へ格下げし **cache_floor = MAX (miss を数えない = 警告対象外)**。
検証の一般化: **キャッシュの live 検証は「同一リクエストの再送」では接地にならない — 
実戦は必ず 末尾 が変わるので、可変部を変えた列で測る**。

### 83. 「参照が意味をなしていない」— 渡す側が正しくても受け取る側に差し込み口が無ければ沈黙する (ComfyUI / Krea 2)

**症状** (2026-08-22 ユーザー報告): 設定画集 (spec 25) を置いても ComfyUI の挿絵がキャラに寄らない。
Kataribe は画集を `/upload/image` へ正しく上げ、`%ref_n%` の置換も正しく動いていた。

**真因は二つ重なっていた**: ①ユーザーの実ワークフロー (`Krea2 Moody.json`) に `%ref_n%` が**一つも無い**
— 上げた画像はどのノードにも繋がらず、エラーも警告も出ない (送る側の成功だけが計器に乗る)。
②rev2 契約「未使用の `%ref_n%` は残す・LoadImage の無効化はワークフロー側の責務」は、**画集の枚数ごとに
ワークフローを貼り替える運用**を暗黙に要求していた。残った `%ref_2%` は LoadImage を File not found で
落とすので、3 枚用のワークフローを 1 枚の盤面で使うと生成自体が死ぬ。誰もそんな運用はしない。

**処方**: (a) 未置換 `%ref_n%` を持つノードを**ノードごと外し、そこへの辺を切る** (`comfy_prune_unfilled_refs`)
＝ワークフロー 1 本が 0〜3 枚を受ける。(b) 設定タブに「差し込み先 `%ref_1%` が無い = 送られても使われない」の
警告 (①の沈黙を見せる)。(c) 同梱テンプレート `comfy_krea2_ref.json`。**←2026-08-26 に (c) は撤去**
(ユーザー決定。ComfyUI を用意できる人はモデルも絵柄も自前で、同梱するとモデルの入手までサポート対象に
見える)。同梱は汎用 1 本だけになり、テンプレートが担っていた「置換語の入れ方の実例」の役目は
**警告 3 本** (`%ref_1%` / `%seed%` / `%negative%`) が引き継いだ — 実例が消えると入れ忘れは
沈黙するので、撤去と警告の追加は対で入れている。

**接地で分かったこと**: Krea 2 は ComfyUI 本体が参照をネイティブ実装している (`model_base.Krea2` が
`reference_latents` を Kontext 同型でシーケンス連結)。IPAdapter 系は不要で、core の
`TextEncodeQwenImageEditPlus` (clip+vae+image1..3) だけで **LoRA 無しの turbo でも識別性が保たれる**
(LAN 実機・同 seed・n=1 目視: 参照あり＝銀髪/緑目/赤マフラー/紺×金鎧を保持、なし＝金髪ロング/青目/銀鎧)。
副作用＝**参照の無地背景が構図に漏れ、灰色の枠つきビネットになる**。プロンプト指示で大きく減るが消えない
→ プロンプト書きの規律に「無地背景・余白・枠・文字を持ち込まず場面の背景を全面に」を追加。

**一般化**: 「渡した」と「使われた」は別の計器で測る。送信側の成功 (200・置換済み) は受信側に差し込み口が
無い失敗を隠す — 受信側の構造 (ワークフローに `%ref_1%` が在るか) を送信前に検査して、沈黙を警告に変える。
#75 (`table_start` の取りこぼし) と同族で、あちらは受信 switch の allowlist、こちらは受信グラフの配線。


### 84. 揮発状態のリセットが 2 箇所に手書きで散っていると、**追加**でも両方落ちる

**症状** (2026-08-23 ユーザー報告): spec 27 で入れた「この一枚への指示」(`imageDirection`) が、
**新しいゲームを始めても残っていた**。仕様 (spec 27 決定 12) にもコミットメッセージにも台帳にも
「新規/ロードで消す」と書いてあり、**書いた本人が書き忘れていた**。

**機序**: 挿絵まわりの揮発物を捨てる契機は 2 つある — ①新規/再開/ロード (`applyGameView`)
②章の遷移 (`play_turn` の transition 分岐)。両方に**同じ列挙が手書きで生えて**おり、spec 27 が
揮発物を 1 つ増やしたとき、その列挙に触れなかった。

**これは #撤去の非対称 (mandate) の裏面**。あちらは「撤去は既存節の読み直しを要求するので落ちる」
だが、こちらは**追加**で落ちている。追加が目に入るのは**その列挙を編集しているときだけ**で、
新しいフィールドを `state` に足す作業は列挙のある場所を一度も開かない。ゆえに
「気をつける」では防げず、**列挙を 1 本にする**のが処方 (`dropVolatileImage()`)。

**一般化**: 同じ規律が 2 箇所に手書きで在る形は、撤去でも追加でも落ちる。**捨てる物のリストは
1 箇所に持ち、契機の側がそれを呼ぶ。** 「片方だけ直った」状態は動作でしか見えず、しかも
**仕様・台帳・コミットメッセージは正しいまま**なので、読んでも気づけない (今回まさにそれ)。

**接地の限界**: frontend にはテストランナーが無く、この修正の Red→Green は**ユーザーの実機観察**
(Red) と目視 (Green) に依存する。列挙を 1 本にしたのは、次に揮発物が増えたとき同じ形で落ちない
ようにするためで、今回の修正そのものを機械で守るものではない。

### 85. 入力を指す語がプロンプトに残ると、画像モデルはそれを**被写体**として描く

**症状** (2026-08-23 ユーザー実測): 参照画像つきで挿絵を出すと、**1 枚の場面ではなく資料集のような
上下 2 分割の画像**が返る (同じ部屋・同じ二人のカットが縦に並ぶ)。

**真因**: spec 25 決定 8 の規律が、プロンプト書きに「登場人物と場所の見た目は参照に従い、
**プロンプト内で `(as in the reference sheets)` と指せ**」と要求していた。書き手の LLM は忠実に
その語を出力へ入れ、**画像モデルはそれを構図の指示として読む** — reference sheet とは「複数カットを
並べた資料」だからである。つまり**入力の説明が出力の説明として解釈された**。

**参照はテキストで指す必要が無かった**。運び方はどれも画像そのものを渡す経路で (Gemini の
`inlineData` / OpenAI `images/edits` の `image[]` / ComfyUI の `reference_latents`)、モデルは添付を
直接見る。指し示す語は情報を足さず、被写体の語彙としてだけ効いた。

**処方** (prompt 層のみ・engine 不変): 規律を**対の 2 箇条**へ。①見た目は髪色・髪型・目の色・服装の
**具体語**で書く／**『参照画像』『設定画集』『reference sheet』のような参照そのものを指す語を書くな**
②**ひとつながりの 1 場面**として描く (`a single continuous scene, one frame, no panels or split screen`)。
②は①の取りこぼしに対する保険で、参照つきのときだけ足す (0 枚なら spec 24 と byte 一致は不変)。

**一般化**: **入力について書いた語は、出力についての語として読まれうる。** メタな指示 (どこを見よ・
何に従え) をモデルへの本文に混ぜると、本文はすべて被写体の記述として解釈される。#28 (narration へ
漏れた tool-call の format token) と同族で、あちらは構造マークアップが本文に混ざり、こちらは
**入力の呼び名**が本文に混ざった。参照・添付・資料のように「渡すもの」がある経路では、
**渡した事実は運び方が伝え、本文は描くものだけを言う**。

**接地の限界**: n=1 (ユーザーの実機 1 枚)。分割が「reference sheets」の語だけで起きたのか、参照画像
自体が資料集レイアウトだったことの寄与があるのかは分離していない (後者なら②が効き、前者なら①が
効く。両方入れたので次の実機で分割が消えても、どちらが効いたかは分からない)。

### 86. 名前が固定で中身が入れ替わる置き場では、URL は識別子として足りない

**症状** (2026-08-23 ユーザー実測): 参照ストックで**どの枠を消しても、末尾の絵が消えたように見える**。
3 枚あるときに 1 番を消すと 3 番が空になり、1・2 番には元の絵が残って見える。

**engine は正しい。** `delete_slot` は 1 番のファイルを消し、後続を前へリネームして詰める
(`ref2→ref1`, `ref3→ref2`)。ファイル**名**は `ref1..k` のまま動かず、**中身だけが入れ替わる** —
これは前詰め不変条件 (spec 27 決定 10) が意図してそう設計したもので、そのおかげで
「UI 番号 = ファイル名番号 = ワイヤ番号」が保たれている。

**真因は提示層のキャッシュ**。サムネイルは `convertFileSrc(dir/ref1.png)` の `asset://` URL で、
生成画像は全部 `.png` なので**削除の前後で URL が 1 バイトも変わらない**。WebView は同じ URL を
キャッシュから出すので、`ref1.png` の中身が A→B に変わっても画面には A が出続ける。結果、
残ったのは B・C なのに画面は A・B を映し、**末尾が消えたように見える**。

**見落とした理由が PoC の中にある**。`delete_compacts_and_preserves_extensions` は
`ref1.png / ref2.jpg / ref3.webp` と**拡張子を散らして**書いていた。その形ではリネームで
ファイル名も変わるので、キャッシュは自然に割れる — **実運用 (生成画像は全部 png) とは違う、
たまたま安全な側だけを固定していた**。同じ拡張子の列を足したら、名前が動かず中身だけ変わることが
テストの字面に現れた (#82/#57 の「観測の形が実戦の形と違う」の再演。今回は観測でなくテストの形)。

**処方**: 変更のたびに繰り上がる版 (`refStockRev`) をサムネイル URL に `?v=` で付けて割る。
バイト長や mtime でなく単調カウンタにしたのは、**同サイズの絵に入れ替わっても必ず割れる**ため。

**一般化**: **安定した名前を持つ置き場で中身が入れ替わる設計にしたなら、その名前を URL に使う側は
必ず版を持つ。** 名前の安定は機構の要求 (ここでは番号の恒等一致) で正しく、キャッシュはその
安定を「同じ絵」と読む。engine が正しいまま画面だけが嘘をつくので、ログにもテストにも出ない。

**接地の限界**: `?v=` が Tauri の asset protocol を素通りする (query はパス解決に混ざらない) 前提。
GUI 実機で未確認。もし 404 になるなら、割り方をファイル名側 (世代つきの名前) へ移す必要がある。

---

### 87. 失敗のたびに次の入力が重くなる — リトライが仕事を膨らませる形

**症状 (2026-08-26 ユーザー実機、長めのセッション)**: あらすじ要約が毎ターン
`タイムアウト (15 秒)` で失敗し続け、止まらない。ユーザーの言: 「長くなると要約の失敗が
多くなる。一度失敗するとどんどんログが溜まっていく」。

**真因は二つ重なっていた。**

**① 15 秒が短い。** `SYNOPSIS_TIMEOUT_SECS` は「受理ターン直後の同期実行がプレイを長く
止めないための上限」として選んだ値だった (spec 10 rev2)。だが遅いモデルでは
**毎ターン 15 秒払って必ず失敗する**という最悪の形になる — 待ち時間の節約が目的だったのに、
一度も成功しないので払った時間が全部捨てられる。**一度 60 秒払って成功する方が安い。**
他の生成経路 (エピローグ 30 秒・挿絵のプロンプト書き 60 秒) から見ても 15 秒だけが外れ値だった。

**② あふれ契機の失敗が範囲を凍結していなかった。** `abandon` は遷移契機だけを
`pending_transition` へ凍結し、あふれ契機は「次の受理ターンで範囲を再計算」していた
(「章を跨がないので安全」)。ところが `next_job` の `end` は
`unc[unc.len() - 1 - KEEP_RECENT].turn` なので、**未圧縮が 1 本増えるたびに範囲が 1 つ伸びる**。
つまり失敗するほど次のリトライが重くなる。使い捨て probe で実測: 5 ターン経過で
`end=11 → end=16`（対象 11 本 → 16 本）。**タイムアウトで失敗した仕事を、より大きくして
やり直していた。**

**処方三点**（①だけでは追いつかれるので対で入れる）:
(a) 既定 60 秒 + `SUMMARY_LLM_TIMEOUT_SECS` で上書き（既存 `SUMMARY_LLM_*` 一族に合わせた env
直読み）。(b) **契機を問わず範囲を凍結**（フィールドを `pending` へ改名・旧セーブは
serde alias で読む）。(c) **同一範囲を 3 回失敗したら機械 join で確定して先へ進む** —
(b) だけだと恒久的な失敗条件（規約ブロック #61・恒常タイムアウト）で永久に詰まり、その間ずっと
毎ターン待たされる。あらすじの質は落ちるが LLM を呼ばないので必ず成功する。
**質の劣化より停止の回避を採る。**

**一般化**: **リトライは、やり直す仕事の大きさを変えてはいけない。** 範囲を「そのつど現在の
状態から計算し直す」設計は、正常時は自己修復的で美しいが、対象が時間とともに増える場では
**失敗が次の失敗を確実にする**正のフィードバックになる。凍結（範囲を固定）と逃げ道（K 回で
劣化版に落とす）は対で入れる — 凍結だけでは詰まり、逃げ道だけでは膨らむ。

**なぜ気づきにくかったか**: 失敗そのものはログに出ていた（警告は毎ターン出る）。出ていなかったのは
**リトライの対象が回を追って大きくなっていること**で、これはどの行にも書かれていない。
症状は「たまに失敗する」に見え、実際は「失敗するほど失敗しやすくなる」だった。

**残る設計上の選択肢（未実施）**: 圧縮は今も `play_turn` の中を `.await` で塞いでいる。
60 秒に延ばした分、成功時の待ちも最大 60 秒になりうる。バックグラウンド化は spec 10 rev2 で
「状態競合・終了時消失・recent_log との瞬間不整合を防ぐ利得の方が大きい」として見送った判断で、
覆すなら別 spec。

---

### 88. 使っていない機構が、既定で口を塞いでいた — Tauri の `dragDropEnabled`

**症状 (2026-08-26)**: 右ペインの顔アイコンを参照ストックの枠へドラッグする経路を実装したが、
実機でそもそも**掴めない**。`dragstart` が一度も発火しない。

**真因**: `tauri.conf.json` の window 設定 `dragDropEnabled` は**未指定＝既定 true** で、Tauri は
OS 側の drop ハンドラ (Windows なら WebView2 の `IDropTarget`) を登録する。これがページ内の
HTML5 ドラッグより先に口を押さえる。**Kataribe はネイティブの drag-drop イベントを一箇所も
使っていない** (`tauri://drag-drop` / `onDragDropEvent` とも grep でゼロ) のに、使っていない
機構のために web の機構が死んでいた。`false` にして解決。

**ユーザーの見立ては半分当たっていた** — 「まだ画面ドラッグを使っている箇所が無いから機構が
開いていないのかも」。正しくは「開いていない」のではなく「**別のものが既定で先に占有している**」。
どちらも「有効化が要る」で処置は同じだが、機序が違うと探す場所が変わる (フレームワークの
権限設定を探すか、window 設定を探すか)。

**予測が当たったので機序も確認できた**: この設定が原因なら、2026-08-24 に入れた
**ローカルファイルのドロップも一度も動いていなかった**はず (クリックのファイル選択でしか
試していなかった)。実機で両方いっぺんに動いた。

**一般化**: **プラットフォーム層が「親切のために」既定で有効化している機構は、使っていなくても
上位の同種機能を奪う。** 症状は「自分の書いたコードが動かない」に見え、自分のコードを何度
読み直しても出てこない — 探す先はフレームワークの既定値である。判定は安い: **その機構に
依存しているコードが 1 行でもあるか grep する。ゼロなら切って失うものは無い。**
...
### 92. `v-for` の中の文字列 ref は配列になる — フォーカスが当たらず入力欄が閉じなかった

**症状**: 編集モードのファイル名入力 (作成・改名) が、クリックで開いたあと**フォーカスされず、
どこを押しても閉じない**。ユーザー報告「フォカースを抜けたらインプットを閉じるのが良い。
開きっぱなしなのは気持ち悪い」。

**機序**: 入力欄は `v-for` の内側に置いてある。Vue 3 は **`v-for` の中の文字列 ref を配列として
集める**ので、`draftInput.value` は `HTMLInputElement` ではなく `[HTMLInputElement]` だった。
`el.focus()` は `undefined is not a function` で例外になり、呼び出し側が `void focusDraft(...)`
で投げっぱなしにしていたので**未処理の rejection として消えた**。結果:
フォーカスが当たらない → **blur が一度も発火しない** → 閉じる契機が無い。

**閉じる処理は最初から書いてあった** (`@blur="commitDraft"` が確定して畳む)。壊れていたのは
その手前で、**症状は「閉じない」なのに原因は「開いていない (フォーカスが無い)」**。
`v-model` は効くので入力自体はできてしまい、手でクリックすれば blur も動く — この
「半分動く」が原因の切り分けを鈍らせる。

**処置**: 関数 ref (`:ref="bindDraft"`) で素の変数へ受ける。同時に開く入力欄は高々 1 つなので
配列を扱う必要はない。畳むときに変数も null に戻す (消えた行の要素を掴んだままにしない)。

**←2026-09-19 自動テストで固定 (spec 31 Phase A)**: `StatePanel.mount.test.ts` が新規作成・改名とも
`document.activeElement` が入力欄であることを見る。修正前の `ref="draftInput"` に戻すと落ち、未処理 rejection に
`el.focus is not a function` が出る (この節の機序そのもの)。

**一般化**: **`v-for` の内側では ref の型が静かに変わる** — TypeScript は
`ref<HTMLInputElement | null>` の宣言をそのまま信じるので**型検査を素通りする**。
`v-if` で 1 つしか出ないことが分かっていても、`v-for` の中に置いた時点で配列になる。
加えて、**`void promise` は失敗を捨てる** — 「呼びっぱなしでよい」のは失敗しても無害な時
だけで、UI の初期化のように失敗が別の症状に化ける処理では catch して見せるべきだった。

## 方法論 (2026-08-28 spec 28 — 提示層の実装バグ 6 件を 1 日で回収した形)

spec 28 (パッケージエディタ) の GUI 目視で、私の実装バグが 6 件出た。**PoC は全部 green の
まま**で、6 件のうち 5 件は**実機でしか出ない形**だった:

| 症状 | 真因 | なぜテストで出ないか |
| --- | --- | --- |
| 起動 panic (#89) | 型エイリアスの TypeId 衝突 | unit テストは Tauri builder を通らない |
| 本文がスクロールしない | `height:100%` の連鎖切れ | frontend にテストランナーが無い |
| 開いただけで ● | CodeMirror の LF 正規化 | 実ファイルが CRLF のときだけ |
| 閉じるボタン無反応 (#90) | フックが実行経路を変えた | capabilities は実行時にしか効かない |
| クロップが SecurityError (#91) | asset:// で canvas 汚染 | カスタムプロトコル越しでのみ |
| 入力欄が閉じない (#92) | `v-for` 内 ref が配列 | 型宣言をそのまま信じて素通り |

**共通の形**: どれも**「机の上では成立する」層**にある。私が組んだ盤面 (unit テスト・型・
純関数) では全部正しく、製品が実際に通る継ぎ目 (起動・WebView・OS のファイル・
カスタムプロトコル・テンプレートのコンパイル結果) でだけ破れる。
`verification_at_the_real_seam_20260822` の「接地の質は観測回数でなく観測地点で決まる」が、
**提示層では観測地点をそもそも私が持てない**という形で現れた。

**ゆえに規律は「もっとテストを書く」ではない**（書けない層の話なので）:
1. **接地の限界を毎回申告する** — 「PoC green」で止めず「機械で守れているのはここまで、
   実機で確かめてほしいのはこれ」を残す。spec 28 は Status 行に目視チェックリストを
   凍結してあり、それが 6 件を出す順路になった。
2. **症状と原因の層がずれることを前提に切り分ける** — #92 は「閉じない」の原因が
   「開いていない (フォーカスが無い)」だった。ユーザーの言葉は症状であって原因ではない。
3. **半分動く形を疑う** — #92 は `v-model` が効くので入力できてしまい、手でクリックすれば
   blur も動いた。完全に壊れているより切り分けが遅れる。

**ユーザー側の観察も記録しておく**: 6 件とも報告は一言 (「スクロールがない」「閉じるボタンが
効かなくなった」「エラーが発生しました」+ スクリーンショット) で、**全件が実在**した
(`kataribe_felt_report_diagnostic_pattern_20260709` の n+1)。体感報告は設計書レビューより
優先するチャネル、という判断は今日も正しかった。

## crates/gm_core (2026-09-01 実プレイ — 「提案者エラー: 構造化出力のパース失敗 … untagged enum OneOrMany」)

**症状**: ターンが丸ごとエラーで落ち、画面には
`data did not match any variant of untagged enum OneOrMany` と raw JSON だけが出る。
raw を読むと構文は正しく、`ops` も配列で、壊れているのは**中身**だった —
GM が `give_meat_to_momoka` / `eat_meat` という**存在しない op 名**を発明していた
(正しくは `give_item` と `remove_item`)。

**真因は三層**。表示されている文言は真因ではない。

1. **本来の失敗**: LLM が幻の op を出した。これ自体は self-repair が直すはずの正常な失敗。
2. **診断が握り潰されていた**: `StateDelta.ops` は #64 の救済で `one_or_many` を通っており、
   その中身が **untagged enum** だった。serde の untagged は全バリアント失敗時に
   **内側のエラーを捨てて**総称文言だけを返す。probe で実測:

   | 経路 | 出るメッセージ |
   |---|---|
   | `one_or_many` 経由 (当時) | `data did not match any variant of untagged enum OneOrMany` |
   | `Vec<StateOp>` 直接 | ``unknown variant `give_meat_to_momoka`, expected one of `add_item`, `remove_item`, `give_item`, … `` (全 19 個) |

   後者が**本来出るはずだった文言**で、無効な op を名指しし有効な op を全列挙している。
   しかも幻 op に限らず、**有効な op のフィールド欠落 (`itme` typo) も同じ文言に潰れていた**
   = `ops` の診断チャネルは #64 以降まるごと死んでいた。
3. **self-repair が誤誘導していた**: `turn.rs` は潰れた文言を再生成プロンプトへ載せたうえ、
   「あなたの前回の出力は **JSON として壊れていて**読めなかった」「**ops は要素が 1 つでも
   必ず配列**」と続けていた。この出力は JSON として壊れておらず ops も既に配列 =
   **LLM が正しくやっていることを指示し、真因に一言も触れない**。ゆえに同じものを出し直し、
   `max_attempts` を使い切ってハードエラーになった。

**処方**: `one_or_many` を untagged enum から手書き Visitor へ (`visit_seq` は要素の失敗を
そのまま伝播、`visit_map` が #64 の単一オブジェクト救済)。**受理する形は untagged 版と同一**
(配列 / 単一オブジェクトのみ。null・文字列・入れ子配列は従来どおり拒否 — probe で前後比較)、
**変わるのは失敗したときに何が言えるかだけ**。あわせて self-repair の文言を「理由を先頭に
立てる」形へ (「JSON として壊れて」の断定と「必ず配列」の注意書きを撤去 — 後者は受け側が
救済するのでもう要らない)。依存追加ゼロ・schema 不変 = プロンプトキャッシュの安定
プレフィックスも動かない。

**一般化 (#78 の再演)**: **緩さが安全なのは情報を捨てないときだけ。**
#78 では `parse` の lenient 二経路が救済の失敗理由を握り潰していた。今回は救済のラッパー
自体が同じことをしていた。**寛容化を入れるときは「失敗したときに何が言えるか」を
必ず前後で比較する** — 受理集合だけを見ていると、診断が死んだことに誰も気づかない
(#64 から今日まで気づかなかった)。

**残る別問題**: 幻 op の発明そのものは schema の制約デコードが効いていない側の話 (#52 と同族)。
ただし診断が戻れば self-repair が「有効な op はこの 19 個」と伝えられるので、実用上は
そこで回収されるはず。頻度は次のプレイで測る。

**PoC**: `llm_client` の `rescue_must_not_swallow_the_reason_a_delta_failed`
(幻 op の名指し + 19 個の列挙 / 救済経路でも真因 / フィールド欠落 / 受理集合の不変)。

## app/src (2026-09-01 実プレイ — 「1 ターンで 2 回ダイスを振ると 2 回目がクリックできない」)

**症状**: 1 ターンに判定が 2 件積まれると、1 個目は開帳できるが**2 個目のカードが押せず**、
入力欄は `hasUnrevealedDice` で締まったまま先へ進めない (デッドロック)。

**真因**: `DiceReveal.vue` の `state` は `idle → rolling` へ一方向に進むだけで `idle` へ戻らない。
2 件が**同じログ行**に積まれると `v-if="entry.revealed < entry.checks.length"` は真のままなので、
Vue は**同じコンポーネントインスタンスを使い回す** — props だけ 2 個目に差し替わり `state` は
`rolling` のまま。ゆえに `@click` は `start()` の先頭 `if (state.value !== "idle") return;` で
弾かれ、表示も 1 個目の出目のまま固まる。**正本も store も無実**
(`revealTargetIndex` / `revealNext` は正しく 1 個ずつ進む設計のまま)。

**なぜ今まで出なかったか**: プッシュの振り直し (spec 18 Phase B) は同じ行を再び伏せるが、
その前に一度 `revealed == length` = **v-if 偽 (unmount) を通る**ので必ず新しいインスタンスに
なる。**唯一 unmount を通らない経路が「同一行に 2 個以上」だった。**

**処方**: 呼び出し側で `:key="`${i}-${entry.revealed}`"` (3 箇所: rolls / checks / statrolls)。
**key は値でなく位置で切る** — props の watch で state を戻す案は、同じ出目が 2 回出たときに
変化を検出できない (2 個の d20 がどちらも 7、は普通に起きる)。

**一般化**: #92 と同じ **Vue のインスタンス再利用**の一族で、これで 2 件目。
「`v-if` が真のまま props だけ替わる」形は、**コンポーネントが内部状態を持ち、かつその状態が
props に対して一方向**のときに必ず壊れる。書く側の規律は「内部状態を持つ提示部品は、
何の 1 個を表しているかで key を切る」。

**接地の限界**: この修正に自動テストは付いていない。frontend の vitest (2026-08-29 新設) は
DOM を持たず (`@vue/test-utils` / `happy-dom` 未導入)、**コンポーネントのマウントを伴う
再利用挙動は現状テストできない**。Vue インスタンス再利用のバグはこれで 2 件目なので、
マウント用ハーネスの導入は起票の価値がある (未決)。

**←2026-09-19 解消 (spec 31 Phase A)**: `ConversationLog.mount.test.ts` が判定 2 件の行で 2 枚目が開くことを
見る (同じ出目 2 回の場合も)。`:key` を消すと `revealed` が 1 のまま止まって落ちる。

## crates/gm_core (2026-09-03 実プレイ — 「焼き魚を持っているのに eat_fish が 4 回却下された」)

**症状**: 所持品に「焼き魚」があるのに `eat_fish` (requires: player が焼き魚を所持) が
`max_attempts` の 4 回とも却下され、ターンが落ちた。合計所持数がちょうど 11 個になった直後
だったので、ユーザーには「正本は 10 個までしか認識しない」に見えた (孤島のサラリーマン
サバイバル、turn 193、砂浜)。

**上限説は棄却**: inventory は `BTreeSet` で個数を見る箇所は engine/prompt/GUI のどこにも無い。
使い捨て probe で 15 個の add_item が全件受理、実セーブの経緯を逆再生すると turn 123〜127 で
合計 11 個が受理されている。

**真因は自壊する束**: 実セーブに対して候補デルタを裁定し直すと、却下されるのは
**`remove_item 焼き魚` を `attempt_challenge eat_fish` より前に並べた形だけ**。spec 09 の逐次
射影で先行 op が魚を消し、`requires` が射影の中で偽になる。GM は「食べる = 手放す」の意味論で
`remove_item` を自分で書き足していた (challenge の `on_success` が既に remove する = 帰結の
二重書き)。

**握り潰していたのは理由の文面**: 却下理由は「必要: player が焼き魚を所持」としか言わず、
**画面でも state_brief でも持っている**ので、LLM には矛盾にしか見えない。壊した犯人 (自分の
先行 op) を名指ししないので、同じ形を 4 回繰り返した。**#50 の「画面は true なのに『true が
必要』で却下」の item 版** — あちらは専権フラグの backstop で真因が名指しされる形に変えたが、
gate 未達 5 種 (item/flag/move/challenge/contest) には同じ穴が残っていた。

**処方 (engine + reason 層のみ・prompt 不変 = キャッシュ無風)**: gate 未達系の 5 変種に
`broken_by: Option<StateOp>` を足し、`validate_ops` が「その gate はターン開始時の state では
真か」を見て、真なら射影に載せた op 列を開始時から再生して**最初に偽へ落とした op** を載せる。
attempt_challenge の共通効果が壊した場合の責めは attempt_challenge 自身。文面は
「ただし、この条件はターン開始時には満たされていた。この ops の先行 op〔op: remove_item,
item: 焼き魚〕がそれを壊している。その op を消すか、順序を入れ替えれば通る」— #42 の
「何がダメかでなく何をすれば通るか」の規律。最初から未達なら従来文面のまま。
PoC: `self_broken_premise_names_the_earlier_op` (ChallengeLocked と MoveGateUnmet の両方で
名指し / 最初から未達は名指し無し / 順序を入れ替えれば受理)。

**一般化**: **逐次射影は「裁定の時点」を op ごとにずらす。ずらした時点で評価した却下理由は、
プレイヤーと LLM が見ている時点 (ターン開始) の事実と食い違いうる**。射影裁定を持つ以上、
gate 未達の理由は「どの時点の評価か」と「その時点まで誰が状態を動かしたか」を必ず載せる。
これは項目を数えるより先に「却下されたデルタの形」を再現する方が早い類型でもあった —
上限説の検証 (probe) は 5 分で棄却でき、真因は候補デルタ 8 形の裁定で 1 発だった。

**接地の限界**: ユーザーの画面に出た却下文言そのものは確認していない (autosave は却下ターンを
...
受理 (PoC で固定)。**実機 Green (2026-09-07 ユーザー実測 n=1)**: モモカに焼き肉を持たせ「モモカの持つ焼き肉を二人で
食べる」→ 二人で食べられた。却下された試行はゼロ = **一発受理 = GM_SYSTEM の接地が効いた** (reason 層の新文面は
この周では発火していない)。モデルは Meta muse-spark 1.3 系で、ツール実行の失敗が元々少ない側
なので、弱いモデルで reason 層が self-repair を一発で通すかは未測定。モデル差の吸収は今はしない
(ユーザー決定)。
...
最終送信文字列の先頭に 2 回続けて現れる。毎回ではない。

**設定は無実**: 設定ミラー (`app_data/settings.json`) の `kataribe.imageGen.userPrefix` は 1 回分。
`prompt_override` の経路もスタイルを前置きしない (verbatim)。

**真因は合成**: 最終プロンプト = スタイル原文 + プロンプト書きの出力 (spec 27 B-1)。プロンプト書きには
素材節で「この文字列は先頭に前置きされるので重複して書くな」と渡しているが、**規律だけで機械の守りが
無かった**。タグ列のスタイルを「画風のタグ」として出力の先頭へ写す個体差が確率的に出て、合成が
`スタイル, スタイル, 本文` を作る。

**処方**: `compose_image_prompt` に `strip_leading_style_echo` — 出力の先頭がスタイル原文と
(空白・大小文字・末尾区切りを無視し非空白文字の列で) 一致すれば、写しと直後の区切りを落として繋ぐ。
語の途中では切らない (`photo` は `photograph` を剥がさない)。override は不変。PoC
`compose_strips_a_leading_echo_of_the_style` (Red は報告そのものの二重文字列を再現)。

**一般化**: LLM に「書くな」と渡した規律は、書かれたときの受け皿が無ければ守られない (#93 の
「緩さが安全なのは情報を捨てないときだけ」の裏面 = **厳しさが効くのは機械が受け止めるときだけ**)。
「見せない (prompt)」と「通さない (機械)」の二層は閉世界の防衛線 (#50) と同じ形で、提示層の
文字列合成にも要る。

**同日追補 — 写しは verbatim ではない (ユーザー実データ)**: 実際の二重文字列は
`Photograph, looking away,  hyper-rea, full body, Photograph, looking away, hyper-real, full body, …` —
プロンプト書きは**スタイルの誤字 (`hyper-rea`) を直して (`hyper-real`) 写す**。最初に入れた
「空白と大小文字を無視した文字列一致」ではこの 1 文字で外れて剥がれない (ユーザーは「空白が
2 つだと一致しない」と読んだが、空白は最初から無視していて、外していたのは誤字修正の側)。
一致を**タグ単位** (`,` `、` `.` `。` `;` 改行で区切る) にし、各タグは正規化のうえ 5 文字以上で
編集距離 `1 + 長さ/10` を許す (短いタグは完全一致のみ = `cat`/`car` を同一視しない)。
PoC `compose_strips_echo_even_when_the_writer_fixed_a_typo` は実データそのものを Red にした。
一般化: **LLM の「写し」は正規化を伴う** — 大小文字・空白・誤字・句読点を揃えてくる。写しを検出
する機械は、完全一致でなく「人が同じと読む」粒度で比べる。

**接地の限界**: 「先頭に 2 回続く」形はユーザー実データで確認済み。編集距離の閾値 (5 文字以上で
1 + 長さ/10) は実データ 1 件に合わせた値で、誤字が 2 箇所以上あるタグは剥がれない。
**実機 (2026-09-03 ユーザー実測)**: 今日の main を `bun run tauri dev` で起動して再生成 → 二重なし。ただし n=1 で
写しが出たかは不明 (「写しが出ても落ちる」と「今回は写さなかった」を区別できない)。実データを固定した PoC が
主根拠で、実機は「壊していない」側の確認。


## app/src-tauri (2026-09-04 実機 — 「編集モードでメディアの削除ボタンを押しても画面が変わらない」)

**症状**: 編集モードのメディア面 (images/ audios/ の一覧) で ✕ を押すと、確認ダイアログは出て
ファイルも実際に消えるのに、一覧からは消えない。ユーザーには「無反応」に見えた。

**真因は返りの形 (spec 28 追補、2026-08-28 に削除をメディアへ広げた時の取りこぼし)**: 削除
command は**作成と同じ返り** (`files` = YAML の一覧だけ) を使い回していた。frontend は返りを
そのまま state に写す設計 (1 往復で UI が揃う流儀) なので、`media` 側の一覧は誰も更新せず
古いまま残った。YAML の削除は `files` が更新されるので正しく動き、**メディアだけが沈黙した**。
リネームは同じ日に「files と media を両方返す」と書いてあった — 削除だけが片側だった。

**処方**: 削除の返りをリネームと同じ `EditorListsView` (files + media) へ。command の本体を
`delete_editor_file_view` に切り出し、返りの JSON に `media` が載り消したファイルが両方の
一覧から消えていることを PoC で固定 (Red = `media` キー自体が無い)。frontend は両方を写す。

**一般化**: **「1 往復で更新後の一覧を返す」流儀は、その command が効く範囲の一覧を全部
返して初めて閉じる。** 経路を広げた (削除がメディアにも効くようにした) ときは、返りの形も
同じ幅に広げたかを数える — 効く範囲が広がったのに返りが元のままだと、広げた側だけが
「実行はされるが画面に出ない」形で沈黙する (エラーもログも出ない = 2026-08-31 の
`dropVolatileImage` と同じ「追加で落ちる」類型)。

**接地の限界**: 実機の目視は未実施 (ユーザー実測待ち)。同じ報告に「参照画像のダイアログも
同様」とあったが、こちらは読んだ限り削除 → 更新後の一覧 + `refStockRev` 繰り上げ → 再描画の
経路が揃っており、一覧と削除が同じフォルダを見ることも確認した。机上で再現できないので
**推測修正は入れていない** (どの表示が古いまま残るかの実測が要る)。
**→ 同日決着**: ユーザーの続報「表示されるまで時間差がある。開いてから読み込む形だと思う」で、
削除の不具合でなく**初回表示の描き順**だった。一覧が届く前に store の `refStock` が null のまま
既定の 3 枠を空で描く = 「参照が無い」という嘘の状態を一瞬見せる。一覧が届くまで読み込み中の
1 行にした (失敗時は失敗と出す)。中身の読み込み量は実フォルダ合計 2.7MB で ms 単位ゆえ真因ではない。

## crates/harness + app (2026-09-06 実プレイ — 「エピローグが生成されない。前はそんなことなかった」)

**症状**: goal に到達しても結末文とバナーだけで幕が下り、エピローグが出ない。エラーもトーストも
出ない。実機目視 3 点 (メディア削除・参照パネル・ウィザード) は全部 OK だった同じ日の報告。

**真因は 2 層**:
1. **30 秒の固定タイムアウトに遅いモデルが間に合っていなかった**。`generate_epilogue` は GM の
   client で素の生成を `EPILOGUE_TIMEOUT_SECS` = 30 秒で切る。live 実測 (`tests/live_epilogue.rs`、
   Meta muse-spark-1.3・908 字の素材・各 3 回) は **24〜51 秒でばらつき**、実プレイの素材量
   (あらすじ 2000 + 経緯 2400 字) ならさらに遅い。30 秒は起草時 (2026-07-14) の Summarizer 15 秒を
   基準に「見せ場だから長め」と決めた値で、その Summarizer 側は 08-26 に 60 秒・09-01 に UI から
   600 秒まで選べるようにしたのに、**ここだけ取り残されていた**。「前はそんなことなかった」は
   GM が Gemini / Claude (速い) から muse-spark へ替わったことで説明がつく。
2. **失敗が沈黙していた**。Err は `eprintln` のみで、リリースビルド (コンソール無し) では何も出ない。
   あらすじには `synopsis-failed` のトーストが在り (2026-07-18)、エピローグには対応物が無かった。
   「生成されない」と「失敗した」の区別がユーザー側に無い形。

**私の最初の見立ては半分外れた**: `.env` に Gemini 検証の `LLM_EFFORT=high` が残っていたので
「effort で思考が長くなり 30 秒を越える」と 1 回目の実測 (high 51 秒 / なし 28 秒) から断定した。
3 回ずつ測ると high 51.2/31.8/28.4・なし 28.3/24.4/47.8 で**差は無く、モデルの素の遅さとばらつき
が原因**だった (2026-08-21 の「n の不足でなく形の問題」と違い、こちらは素朴に n=1 の断定)。
処方は変わらない (どちらでも 30 秒は足りない) が、**支持する結果が出た直後に n を積む**規律を
また踏んだ。

**処方**:
- `EPILOGUE_TIMEOUT_SECS` 30 → **120** (= `request_timeout` の既定と同値、それ以上は API 側が先に
  切る) + `epilogue_timeout_secs()` = 既定と **あらすじの待ち時間設定** (`SUMMARY_LLM_TIMEOUT_SECS`、
  設定 UI の select) の大きい方。専用の env は増やさない — 答えは同じ「どれだけ待てるか」で、
  ユーザーが遅いモデルに 300 秒を選んだなら終幕をそれより短く切る理由が無い。終幕は**次のターンを
  止めない**ので、待ち時間の代償が最も小さい場所。実経路は 28 秒で通過 (live)。
- app: Err で `epilogue-failed` イベント → トースト「エピローグの生成に失敗しました（結末文で幕）:
  {理由}」(ja/en、`synopsis-failed` 同型)。
- **配線の固定**: backend が `.emit("…")` で出すゲームイベント名は frontend の中継表 `GAME_EVENTS`
  (transport.ts) に載っていなければ `LocalTransport.onEvent` が購読せずホストもゲストへ中継しない =
  **emit しても誰にも届かずエラーも出ない**。app backend のテストが両ファイルの本文から名前を
  機械抽出して ⊆ を固定 (`every_emitted_game_event_is_forwarded_by_the_frontend_transport`、
  Red = `epilogue-failed` が表に無い → frontend を足して Green)。手で写した一覧は写し漏れの側に
  穴を開けるので、抽出は本文から。**抽出器の自己検査**も入れた (既知の名前 1 つが拾えていること =
  0 件で緑になる形を許さない。初回は自分の doc comment の `.emit("...")` を拾って `...` で落ちた =
  抽出器は n=1 の実出力で確かめてから固定する、09-04 の教訓の再演)。

**一般化**:
- **同じ役割の定数が複数在るなら、片方を動かしたとき他方も数える**。Summarizer の 15→60 (08-26)
  と UI 化 (09-01) は「遅いモデルに固定秒数が短すぎる」という同じ診断から出たのに、同じ診断が
  当てはまるエピローグの 30 秒には触れなかった。定数を上げる commit で `grep TIMEOUT_SECS` を
  一度回せば済んだ (掟の「撤去したら grep」の**変更**版)。
- **失敗経路の可視化は経路ごとに数える**。あらすじにトーストを付けた日 (07-18) に「eprintln だけ
  の失敗経路は他に無いか」を grep していれば、エピローグは同日に塞げた。`eprintln!("[警告]`
  で grep すると残りが見える (今日は数えていない = 次の候補)。

**接地の限界**: live は Meta 1 モデル・素材 908 字・n=3 で、実プレイ相当の素材量 (4400 字超) では
測っていない。**→ 同日ユーザー実プレイで Green** (エピローグが生成された = 実プレイの素材量でも
120 秒に収まった。n=1)。トースト表示 (失敗側) は実機では未観測 — 失敗が起きていないので正しい。
足りない環境が出たら設定 UI の select を上げる道が既に在る。

**→ 同日の棚卸し (ユーザー指示「沈黙する失敗の棚卸しに着手」)**: 一般化の 2 点目を実行した。
`eprintln!` は app backend に 21 箇所、うち 8 は live テストの出力・1 は seed の情報行・残り 12 が
失敗経路。frontend の空 catch は約 45 箇所あるが全部「Tauri 外 / 壊れた localStorage / placeholder
が空になるだけ / 補助経路 (編集モードの層 2 診断・補完)」で、ユーザーに見えるべき失敗を隠している
ものは無かった。backend 12 箇所の分類:
- **直した 3 類型 (どれも「動くが期待した効果が出ない」形で沈黙していた)**: ①**オートセーブ失敗
  ×4** (受理ターン / 決断 / 対決ラウンド / 既成事実編集) — 進めたつもりの進行が次回起動で消える =
  spec 07 が守るはずのものを静かに失う。3 箇所に複製されていた 9 行のブロックと `facts_autosave`
  を 1 つの helper `autosave(app, sess)` に集約し `autosave-failed` → トースト (配線テストが
  Red→Green で frontend を強制した = 上の機構がそのまま効いた) ②**出所メタが書けない / tree_hash
  が計算できない** (取得時 2 経路 + 更新時 1 経路) — パッケージは使えるが「更新あり」が永久に来ない。
  `InstalledPackage.warning` / `UpdateResult.warning` に載せて frontend がトースト (更新は完了
  トーストと同じ行に併記 = トーストは 1 枚) ③**参照画像の合計上限での切り捨て** — 参照ストックの
  一覧は 1 枚ごとの省略理由しか出せず、合計 12MB での後ろからの切り捨ては生成時にしか決まらない
  ので「参照が効かない」(#83 と同じ症状) としか見えなかった。`GeneratedImageView.dropped_refs`
  → トースト。
- **据え置いた 4 箇所 (理由つき)**: `settings_store` の壊れた settings.json を無視 (起動時・復元を
  諦める側に倒す安全な既定で、鳴らす先の UI がまだ無い) / `rename` の WebView 設定引き継ぎ失敗
  (旧インストール限定の一度きり・app_data 側の告知は理由を載せている) / `allow_directory` の失敗
  ×2 (asset scope。背景が出ないだけで稀・鳴らしても対処が無い) / 一時ファイルの `remove_file`
  群 (後始末の失敗は次回起動の `cleanup_leftovers` が拾う)。
- **数え方**: `grep -n 'eprintln!' app/src-tauri/src/*.rs` と `grep -rn 'catch\s*{\s*$' app/src`。
...
## app/src (2026-09-13 実機 — 「編集モードの補完で日本語を先読み変換すると前半＋前半後半と二重になる」)

**症状**: エディタで値欄 (`item:` / `to:` / `key:` などの id) に日本語を打ち、IME で変換を確定して
補完候補を選ぶと、確定した「前半」が残ったまま候補の全文「前半後半」が後ろへ挿さり
「前半前半後半」になる。ASCII の id では起きない。

**真因は二つの継ぎ目**: ①`@codemirror/autocomplete` は **IME の変換が確定した瞬間に補完ソースを
再実行する** (`compositionend` で `startCompletionEffect`。変換中に本文が動いてカーソルも
移ったときだけ)。②`makeCompletionSource` が「打ちかけの語」を **ASCII 識別子の正規表現**
(`[A-Za-z_][A-Za-z0-9_]*$`) で切っていたので、再実行の位置 (「前半」の直後) では語が空になり
`from = pos` に落ちる。`validFor` は `[^\s:,{}[\]]*` で日本語を通す形だったので、**`from` と
`validFor` の文字集合が別々**で、再実行のたびに置換範囲だけがずれた。値欄を開く契機が「`: ` の
直後」だった (2026-08-28) ので、ASCII の id と `: ` 直後から打ち始めた日本語では `from` が
たまたま正しく、**再実行で `from` を取り直す経路** = IME 確定のときだけ露見した。

**処方**: 打ちかけの語と `validFor` を**同じ定数**から作る (`KEY_WORD`/`KEY_VALID` と
`VALUE_WORD`/`VALUE_VALID`)。値欄は `validFor` と同じ文字集合で切り、引用符は語に含めない
(`"焼き` で開き引用符ごと置換すると候補の前から引用符が消える)。キー欄は型由来で ASCII のまま。
PoC: `makeCompletionSource` を `CompletionContext` + `EditorState` で直接呼ぶ frontend vitest
3 本 (日本語の打ちかけ = Red / ASCII の対照群 / 引用符)。**IME そのものはテストで再現できない**
(DOM も composition イベントも無い) が、ライブラリが再実行することは dist の実読で確定しており、
再実行後のソースの返りは純粋関数なので、そこを固定すれば十分。

**一般化**: **「結果を使い回してよい範囲」と「置換の起点」は同じ文字集合で持つ。** 別々に
持つと、範囲の内側にいる限り古い `from` が生き、範囲を出て取り直した瞬間に `from` が動く —
壊れるのは「取り直す契機」を持つ入力 (IME・貼り付け・アンドゥ) だけなので、ASCII で打つ
作者のテストでは一度も出ない。日本語の id を書く盤面は同梱 content にもあるが (焼き魚・好感度)、
私自身が補完を IME で使う経路を一度も通していなかった = 提示層はユーザーが検出器になる
(2026-08-28 の観察) の n+1。

## app/src (2026-09-13 実機 — 「保存 + 登録モデルの更新を押すと単価が消える」)

**症状**: spec 30 Phase C の単価 3 欄を打って「保存 + 登録モデルを更新」を押すと、登録に単価が
残らず、フォームも空に戻る。エラーは出ない。

**真因**: `saveLlmAndProfile` は **先に `.env` を書く `saveLlm()` を呼び**、成功してから登録を
書き換える (失敗した値を「登録済み」にしない順序)。その `saveLlm()` の末尾にある
`syncSelectionToConfig()` (= 現在の `.env` と一致する登録を選択状態にする同期) が、Phase C で
**単価のフォームも登録の値から引き直す**ようになっていた。同期が走る時点の登録は**まだ更新前**
(初回は単価なし) なので、打ったばかりの 3 欄が空で上書きされ、その直後の更新処理が**空になった
フォーム**を読んで `pricing: undefined` を書いた。

**処方**: ①単価は `.env` を書く**前**にフォームから取り出す ②同期は**選択が別の登録へ移った
ときだけ**フォームに触る (同じ登録のまま通る経路では触らない) ③3 欄揃わず保存しなかったときは
状態行で告げる (部分見積もりを作らない規則を黙って適用しない)。frontend のみ。

**一般化**: **「保存する → 現在値から表示を同期し直す」の順で組んだ保存経路に、あとから
「表示だけに在る入力」を足すと、同期が未保存の入力を消す。** 同期は「何が変わったか」を見て
差分だけ触るか、保存する値を同期の前に取り出す。マウントテストが無い提示層なので (2026-09-01 の
未決) 検出はユーザー実機だった。

**←2026-09-19 自動テストで固定 (spec 31 Phase B)**: `SettingsDialog.mount.test.ts` が AI モデルタブを立て、
単価 3 欄とコンテキスト長を打って「保存 + 登録モデルを更新」を押したあと、入力欄の値と localStorage の登録の
両方を読む。修正前 (61513cd^) の形に戻すとフォーム `'' ≠ '3'` で落ちる。

## app/src (2026-09-14 実機 — 「思考の長さや出力上限を書き換えて『保存 + 登録モデルを更新』を押すと価格とコンテキスト長が消える」)

**症状**: 登録モデルの思考の深さ (effort) や出力上限を変えて「保存 + 登録モデルを更新」を押すと、
単価 3 欄とコンテキスト長のフォームが空になる。エラーは出ない。#103 を直した翌日に同じ操作系で出た。

**真因**: #103 の処方②「同期は選択が別の登録へ移ったときだけフォームに触る」の実装が
`changed = (hit?.id ?? "") !== selectedId` で、**「どの登録にも一致しない」も『移った』に数えていた**。
effort / max_tokens は `profileMatchesConfig` の突き合わせ対象 (2026-09-10) なので、それを変えて
.env を書いた瞬間、登録はまだ古い値を持ち、一致が外れる → hit なし → changed → フォームを空の値で
上書き。登録簿には `saveLlmAndProfile` が先に取り出した値が書かれるので**その回は残る**が、画面は
空のまま = 次にもう一度更新すると本当に消える。#103 の Red は「単価だけ打った」形 (= 一致が外れない)
だったので、この経路はテストの射程外だった。

**処方**: ①判定を純関数 `selectionAfterSave` (`app/src/aiProfiles.ts`) へ出し、フォームを差し替えるのは
**別の登録に一致したときだけ** (一致なしは差し替えの理由にしない) ②「保存 + 更新」は `saveLlm(false)`
で同期そのものを飛ばす (書き換える前の登録簿で同期すると、偶然一致した**別の**登録へ選択が飛ぶ)。
vitest +4、Red は報告の形 (effort だけ変えた config) で `fill` が null にならないことを確認。
**副産物**: `saveLlm` に引数を足したら `@click="saveLlm"` がクリックイベントを `sync` に渡していた
(オブジェクトは truthy なので動いてはいた) — vue-tsc が拾った。テンプレートは `saveLlm()` と明示する。

**一般化**: **「変わったか」を「一致するか」で判定すると、いま自分が書き換えている最中の対象は必ず
一致しなくなる。** 差し替えの条件は「別の何かに一致した」の肯定形で書く。**直した不具合の Red は
実際に報告された操作だけでなく、同じ判定を通る隣の操作 (今回は突き合わせ欄の変更) も含める。**

**←2026-09-19 自動テストで固定 (spec 31 Phase B)**: 同じテストの 2 本目が思考の深さを high にしてから押す。
61513cd の形に戻すと**この 1 本だけが、フォームで落ちて登録簿は通る** = 報告の「登録簿には残るのに画面が空」を
そのまま再現した。純関数の vitest では画面の入力欄までは見えていなかった。

## app/src (2026-09-14 ユーザー要望 — 「メディアのファイルリストでダブルクリックでファイル名編集ができるようにして欲しい」)

**症状**: 編集モードのメディア一覧で、ファイル名をダブルクリックしても名前の入力欄にならない。
画像は全画面プレビューが開いて閉じるだけ、音声は鳴って止まるだけ。エラーは出ない。

**真因**: **ダブルクリックは 2026-08-28 から配線済みだった** (`@dblclick.prevent="startRename"`)。
届いていなかったのは、同じ要素の `@click` が**即座に**プレビューを開いていたから — 画像は 1 回目の
クリックで全画面の幕 (`fixed` の拡大表示) が被さり、2 回目のクリックがその幕に当たって幕を閉じ、
`dblclick` は名前のボタンでなく幕で起きる。音声は 1 回目で再生・2 回目で停止し、`dblclick` 自体は
届くが体感は「押しても何も変わらない」。テキストファイルの行はクリックが「開く」だけで幕を出さない
ので、同じ配線でも改名が効いていた = **同じ流儀を写したつもりの二つの一覧で、クリックの副作用の
大きさだけが違った**。

**処方**: 名前のクリックを 300ms 遅らせ、`dblclick` (と `event.detail > 1`) で取り消す。サムネイルの
クリックは改名の入口ではないので即時のまま。あわせて、要望の後半 (改名したら参照している YAML も
変わる) を `editor::rewrite_asset_references` で入れた (詳細は CLAUDE.md spec 28 リネーム追補 /
data_contract `package_editor.rename`)。

**一般化**: **`click` と `dblclick` を同じ要素に置くとき、`click` の副作用が画面を覆う (モーダル・
全画面・フォーカス移動) なら、`dblclick` は構造的に届かない。** 配線があることは届くことを意味しない
— 検証は「ハンドラが書いてあるか」でなく「2 回目のクリックがどの要素に落ちるか」で見る。
マウントテストの無い提示層なので (2026-09-01 の未決) 検出はユーザー実機だった。

**←2026-09-19 半分だけ自動テストで固定 (spec 31 Phase A)**: `StatePanel.mount.test.ts` が「300ms 経つまで
プレビューは開かない」「間の dblclick で改名になり開かない」を偽のタイマーで見る (即時プレビューに戻すと落ちる)。
**幕が 2 回目を奪う様子そのものは測れない** — happy-dom は hit-testing をしないので、固定したのは取り消しの機構だけ。

## crates/llm_client (2026-09-21 spec 32 — live が「私が質問文から一文落としたこと」を捕まえた)

### 104. 判断モデルへの質問文は prompt と同じ接地の文言 — 一文の増減で確度が 0.36 動く

**観察**: Jev (spec 32) の live テストを書いたら `absent_speaker` が **0.51** で落ちた。
同じ盤面・同じ語りを PowerShell の使い捨てスクリプトで測ったときは **0.93〜0.95** だった。

**真因**: 私が Rust 側へ書き写すとき、質問文から
**「話題に出るだけ・名前が出るだけで台詞を話していない人物は数えない。」の一文を落としていた**。
その一文を戻すだけで **0.51 → 0.87**。state も語りも criteria も変えていない。

**一般化**: 判断専用モデル (Choice / Score / Noul しか返さない類) の質問文は、
**GM_SYSTEM と同型の「接地の文言」**であって設定値ではない。
- 精度はモデルの性能でなく**問い方**で決まる区間がある (この一文の有無で 0.36)
- しかも**失敗が静か** — 質問は通り、答えも返り、値が少し低いだけなので、
  実データと突き合わせない限り劣化に気づけない
- ゆえに**文言は PoC で固定し、変更したら実データで再測定する**。
  Lorekeel が `gm_system_demands_...` で prompt の文言を固定しているのと同じ規律を、
  ここにも当てる (`question_text_is_pinned_to_the_measured_wording`)。

**独立観察 (n=2)**: 同日に X で流れた第三者の実測 (経費申請 20 件を Jev に検査させた) も
「まとめて聞くと正解率 6 割、質問を分けたら 95%」と報告していた。問い方の設計が精度を支配する、
という同じ現象を別ドメインで観測している。

**この罠を捕まえたのは live テスト**で、単体テストは wire の形しか固定していないので素通りした。
2026-09-06 に spec 29 で立てた規律 (#99 / #100 — **新しい経路を初めて使う機能には live を 1 本**)
がそのまま効いた実例。live を書かなければ、劣化した質問文が Phase A の観測データを汚したまま
「Jev の精度はこの程度」と誤った結論を出していた。

**副次 (同日、小)**: frontend の mount テストで私の期待が外れた — backend (`run_consistency_check`)
が既にスコア降順で渡すのに、テストの入力を昇順で組んで「frontend が並べ替える」ことを期待した。
**frontend は並べ替えない**のが正しい (並べ方を決める場所が 2 つあると必ずずれる)。
テストを書くときは「どちらの層が責務を持つか」を先に決める。

## crates/harness (2026-09-21 spec 32 Phase A — 実プレイ 43 ターンで問いの前提が盤面に裏切られた)

### 105. 判定の問いは「盤面のジャンル」に前提を置いている — 短い実測では確かめられない

**観察**: 一貫性検査の `profile_mixup` (「ある人物が、`character_profiles` では**別の人物の
特徴として**書かれているものを見せているか」) は、設計時の実測で 正常 0.15〜0.21 /
違反 0.77〜0.93 と分離していた。ところが実プレイをエンディングまで回すと **中央値 0.30・
真相に近づくターンで 0.5 超えを 5 回**出し、**クライマックスの直前に ⚠ が出た**。

```
…あかりが無意識に身につけていた銀の指輪と同じ歪みだ。
「その指輪……あかりのと、同じだ」
── ⚠ 一貫性検査: 別人の特徴の混入 0.63 ──
```

**真因**: Jev は正しく答えている。**ミステリでは特徴の共有が真相そのもの**で、同じ指輪・
同じ癖・面影は作者が意図的に置いた手がかりだった。しかも両方の profile に書いてあるので
設定どおり。私が設計時に作ったテストケース (筆村が砂糖二つ = 玲子の癖) と、この物語の
「あかりと玲子が同じ指輪」は、**この問いからは同じ形に見える**。

**一般化**: 判定モデルへの問いは、明示していない**ジャンルの前提**を必ず含む。
- 「別人の特徴が現れたら誤り」は、**登場人物どうしが無関係な盤面**でしか成り立たない
- 血縁・双子・師弟・憑依を扱う盤面では、**特徴の共有が筋書きの中心**になる
- ゆえに**短い語り 3 本の実測では絶対に見つからない** — 物語が伏線を回収する段階まで
  回して初めて、問いの前提が盤面に裏切られる

**処方**: 問いを作ったら「**この問いが偽になる盤面はどんな物語か**」を先に一つ挙げる。
挙がるなら、その盤面で必ず鳴ることを織り込んで設計する (advisory にする / 論点を替える)。

**同時に踏んだ二つ目**: 直そうとして**厳密化したら悪化した**。「その人物自身の profile に
無く・別の人物の profile にあり・共有なら除外」と条件を三つ載せた版は、偽陽性も真陽性も
一緒に中央へ寄せ、**判別力を半分にした** (ギャップ 0.50 → 0.25)。**厳密化は精度を上げる行為に
見えて、この器では論点を増やす行為**だった — #104 と同じ「1 質問 1 論点」の規律に、
規律を守っているつもりで反していた。論点を 1 つに削った版 (「人物像から外れた振る舞いを
している人物がいるか」) が分離する側 (0.12〜0.38 / 0.56〜0.67)。

**三つ目 (記録の設計)**: jsonl に**セッション識別子が無かった**ので、43 行が実は 4 セッション
混在で `turn` が 4 回リセットしていた。**action を並べて初めて気づいた**。計器の目的が
「何ターンに 1 回鳴るか」の測定なのに、**分母を数える手段が無い**状態だった。
追記だけの計器には、時刻とは別に**束ねる鍵**が要る。

## 横断 (2026-09-23 — 入力コストの実測と、Jev を剪定に使わない判断)

### 106. 剪定できるのは「今わかる無関係」だけ — 語りの不要は未来が決める

**動機**: ランニングコストの改善。Fuseforks が [Spec 59](../Fuseforks/specs/59_jev-tool-result-pruning.md)
で判断専用モデル Jev による**ツール結果の段落剪定**を Done にし (正味 節約 $0.019 対 Jev $0.0024 =
**約 8 倍の黒字**)、同じ手を Lorekeel の可変プロンプトへ写せるかを測った。**実装は 1 行も書いていない** —
測定だけで見送りを決めたので、本項はその測定の記録。

#### まずコスト構造を実測した (spec 30 の計器が出した最初の実データ)

`app_data/logs/usage.jsonl` 118 行 (7 セッション) と実セーブから:

| | n | prompt | cache_read | $/req | 円/req |
|---|---|---|---|---|---|
| GM `claude-opus-4-6` | 5 | 25,413 | 10,368 | 0.1144 | **17.7** |
| GM `muse-spark-1.3` | 58 | 14,123 | 1,377 | 0.0015 | **0.2** |

- **GM の入力が総 prompt の 94%** (946k / 988k)。出力は入力の 6% で無視できる
- **モデル選択が 88 倍の差**を作る。総額 $0.779 のうち **opus の GM が $0.572 = 73%**、たった 5 リクエストで
- **`muse-spark` の `cache_read` は 58 リクエスト中ほぼ全部 113 トークン** (安定プレフィックス 6,500 超が
  毎ターンフル課金。たまに 7,793 が出るので確率的に効く = failures #82「Meta は命中が不安定」の再確認)。
  単価 $0.1/M なので金額の害は小さいが、**機構としては効いていない**
- opus (Anthropic ネイティブ) は `cache_read` 17,954 / prompt 30,000 = 静的側は多段 breakpoint で守られており、
  **削る余地は非キャッシュの 12,000〜14,600 トークンにしかない**

可変側の内訳 (盤面 `ui_and_ruka` turn 319):

| 節 | 元データ | 実注入 |
|---|---|---|
| **`recent_narrations`** | 8 本 / **7,867 字** | 予算 8,000 にぶつかって全量 (**非キャッシュ側の 65〜75%**) |
| `history_note` | 319 件 / 27,465 字 | 予算 2,400 でクランプ |
| `synopsis_note` | 28 章 / 8,301 字 | 予算 2,000 (第二段でキャッシュ済み) |

**2026-09-13 の見積もりが外れていた** — 直前 K ターン逐語を入れたとき台帳に「1 本 300〜800 字 =
1〜2k トークン程度」と書いたが、実データは **1 本 607〜1,558 字 (平均 983)**。しかも利用者が K を 8 に
上げていたので、可変側の最大項になっていた。

#### 測定 3 段 (`jev-1.13.0`・ブロック割りは語りの境界を跨がず連続段落を 350 字上限で束ねる)

**(1) 写経元の問い「次の語りに必要か」= 関連度 1 軸** — 29 ブロックのスコアが **0.16〜0.48 に固まった**
(median 0.25)。Fuseforks では両端が 0.01〜0.12 と 0.85〜0.94 に割れたのに、ここは**全部が中間帯**。
しかもその帯は、あちらが「**束ね方を変えると 0.17〜0.29 動く**」と名指しした文脈依存の帯そのもの。
**長さとの相関 +0.508** = Jev は「必要か」でなく「長いか」を主に答えていた (短い断片 33 字 → 0.17 /
83 字 → 0.19 / 128 字 → 0.18。Fuseforks の「ディレクトリ項目は依頼が名指ししても 0.20」と同じ現象)。

**(2) `recent_narrations` の実際の目的に合わせて 2 軸へ** — prompt の実文言は「既に確立した静的な情景を
再び描写しないこと」+「ここに書かれた細部は確定した出来事として扱い、食い違う語りをしないこと」なので、
①`repeat` (静的な情景を含むか) ②`commit` (後の語りが従うべき確定した細部を含むか) に割った。
**spread が 0.32 → 0.85 へ跳ねた** (#104 の「一文で 0.36 動く」の再演。今回は問いの軸を替えたので 0.53)。
ところが「どちらにも寄与しない = 落としてよい」断片を論理積で取ると **29 個中 1 個・258 字・3.3%**。
収支は 0.22 円の削減に対し Jev 0.11 円 = **2 倍**で、Fuseforks が `MIN_REDUCTION = 0.25` の門で止める帯に
完全に入る。**理由は Jev の精度ではない — 語りには「情景も描かず何も確定させない断片」がほとんど無い。**

**(3) 読解のみの問いを 4 つ足し、6 問 × 6 サンプル (5 パッケージ・turn 33〜354・ジャンル別) で再現性を見た** —
利用者の指摘「固有名詞が含まれているか、とか聞き方が大きいのだろう」を受けての探索。spec 32 の実測でも
**読解のみの問いが最も強い** (語りだけの移動 0.85 > 読解+照合 0.62 > 読解+判断 0.41) ことが分かっていた。

| 問い | median の幅 | 長さ相関 | 単独閾値での削減率の幅 |
|---|---|---|---|
| `proper_noun` | 0.85–0.96 | 一貫して + | 4 個だけ (**語りは人物名で埋まっている**) |
| `dialogue` | **0.27–0.98** | + | — (サバイバル物は 0.27 / NTR 物は 0.98) |
| `new_event` | **0.41–0.91** | +5 / −1 | th=0.5 で **8% 〜 56%** |
| `sensation_only` | 0.07–0.33 | **−5 / +1** | th=0.3 で **8% 〜 64%** |
| `commit` | 0.18–0.54 | +0.51〜+0.74 | — (長さに引っ張られる) |

- **固定閾値は成立しない**。同じ閾値が盤面によって語りを半分消したり何もしなかったりする (**7 倍の幅**)。
  これは #105「判定の問いは盤面のジャンルに前提を置いている」の再演で、**今回はジャンルを変えた 6 サンプルで
  先に捕まえた** (#105 は実プレイをエンディングまで回して初めて出た)
- **相対順位 (下位 N%) に切り替えれば制御できる** (下位 30% で 22〜29% に収まる) が、それは
  「**落とすべきものが無い盤面でも必ず 30% 落とす**」という性質になる。しかも Lorekeel には
  Fuseforks の `omitted` にあたる**戻る道が作れない** — GM の提示ツールは `emit_delta` 1 本で、
  合成ツールを足すと schema が変わり**キャッシュの安定プレフィックスが動く** (spec 13/14 の資産を壊す)
- `sensation_only` だけが**長さ相関が一貫して負** (5/6) = 長さに引っ張られない唯一の問いだが、
  median が 0.07〜0.33 と低い = 落ちる量も少ない

#### 見送りの根拠 (opus 1 ターン `ui_and_ruka`)

| 手 | 削減 | Jev のコスト | 正味 |
|---|---|---|---|
| **`LOREKEEL_RECENT_TURNS` を 8 → 3** | 4,375 字 | 0 | **3.7 円** |
| Jev 下位 30% (K=8 のまま) | 1,773 字 | 0.055 円 | 1.5 円 |
| Jev 下位 30% (K=3 の上に重ねる) | 約 900 字 | 0.055 円 | **0.8 円** |

収支そのものは立つ (27 倍の黒字)。**だが K を下げるほうが 2.5 倍効いて、しかも無料。**
K を下げた後に Jev を重ねて得られるのは **opus 17.7 円の 4.5%** で、外部 API 依存・盤面ごとの較正・
**落とした語りが戻らないリスク**を背負う対価として釣り合わない。

#### 一般化 (利用者の定式化が最も正確)

> Fuseforks はツール実行出力という**定型的で、何が必要でないかある程度わかるもの**。
> Lorekeel は**必要でないものはストーリーが進んでみないと分からない**。

**これは入力の「形」ではなく「時制」の問題。**

- ツール出力の無関係さは**その場で判定できる** — 依頼文と照らせば、この段落が答えに寄与しないと今わかる
- 語りの不要さは**未来が決める** — この断片が後で参照されるかは、**まだ書かれていない語り**が決める

ゆえに Jev が 0.16〜0.48 に固まったのは失敗ではなく、**正しく「まだ分からない」と答えていた**。
判断専用モデルは現在の入力を読んで答えるので、**答えが未来にある問いには原理的に答えられない**。

**処方**: 剪定を検討するときは、まず「**この入力の無関係さは、今この瞬間に判定できるか**」を問う。
できないなら、問いをいくら練っても分離は出ない (練れば spread は動くが、それは別の軸を測っているだけで、
落としてよいかを測ってはいない)。効くのは**外部から来た定型的な入力**で、**自分が書いたものを自分で
剪定する**のは筋が悪い。

**副次の教訓**: 同じ材料・同じ問いの再測定で `repeat` の median が 0.55 → 0.46、時間相関が
−0.021 → −0.137 に動いた。**1 回の測定で相関係数を語ってはいけない** (Fuseforks も「同じ束ねでの
振れは 0.04〜0.05」と測っている)。私は一度「時間相関がほぼゼロ = K では代替できない」と主張し、
再測で撤回した。

**接地の限界**: 6 サンプル・各 1 回測定 (上の振れを見たとおり n=1 の相関は弱い)。閾値は較正していない。
**落とした結果 GM の語りが壊れるかは一度も実プレイで測っていない** — 本項が言えるのは「削減量が割に
合わない」までで、「落としても安全か」は未検証のまま。ブロック割り (350 字上限) は私が決めた形で、
別の割り方なら分離が変わりうる (Fuseforks の D4 は束ね方で 0.17〜0.29 動くと実測している)。

**後日談 (同日)**: 本項の測定の副産物として **`muse-spark` のキャッシュが 89% 読み損ねている**ことと、その真因が **history の不在** (Fuseforks のログで hist=0 なら 8%・1 件でもあれば 63%) であることが分かり、**`specs/33_narration_history_as_messages.md` を起票した** (直前の語りを可変 user の逐語から assistant メッセージへ移す)。**ただし動機は金額ではなく「同じ情報を 2 箇所に持たない」構造の解消** — 金額だけなら `LOREKEEL_RECENT_TURNS` を 8→3 にするほうが 3.7 円/ターンで効く、という本項の結論は変わらない。

**後日談 2 (同日、spec 33 P0)**: 上の「真因が history の不在」は **P0 の live 測定で支持されなかった** (#107)。history のある本案形でも 1 周目は 113 だった。

## 横断 (2026-09-23 — spec 33 P0: 滑る窓はキャッシュのプレフィックスを伸ばさない)

### 107. 他プロジェクトの相関を移すときは「その入力が伸びるのか滑るのか」を先に揃える

**観察**: spec 33 は Fuseforks のログ (history 0 件で 8% / 1 件以上で 63%) を根拠に「直前の語りを
assistant メッセージへ移せば Meta のキャッシュが効く」と見込んだ。P0 で実セーブ (turn 319 / 354) から
連続 2 ターンの messages を 3 形で組んで実 API へ投げた (`crates/harness/tests/live_spec33_p0.rs`):

- **OpenAI `gpt-5.6-terra`**: t2 が前ターンから読めたのは 3 形とも **system 部分だけ** (11,141 / 12,113)。
  窓の先頭を固定して末尾だけ伸ばした対照 (B-grow) でも**共通の履歴部分まで延びなかった**
- **Meta `muse-spark`**: 1 周目は数秒後の**同一再送すら 113**。約 10 分後の 2 周目は同じリクエストを
  初回からほぼ全量読み、途中までの一致も読んだ (B-grow t2 = 13,169 = system + 共通 3 ターン)

**仮説棄却**:

1. **「history があれば読める」は棄却** — 1 周目の本案形 (history 3 組) は 113。Meta の差を作っていたのは
   **書き込んでから読めるまでの遅延**に見える (数秒 miss / 約 10 分 hit、**推定**)
2. **設計の内部矛盾** — spec 33 D1 は「K と字数予算の上限をそのまま使う」としたが、K 固定の窓は毎ターン
   先頭が落ちるので、次ターンと共通のプレフィックスは **system で止まる**。Fuseforks の history は
   **同じタスクの中で伸びる** (append-only) ので、**滑る窓には移らない性質**だった
3. **OpenAI は伸ばしても読まない** — 前リクエストの途中の位置 (共通の履歴の末尾) は再利用されず、
   既に確立した境界 (system) までしか読めないように見える (**推定**・機構は未確認)

**一般化**: **他所の実測を自分の設計に移すときは、数字より先に「入力の成長の仕方」を揃える。** キャッシュの
プレフィックスは「前のリクエストの頭が次のリクエストの頭と同じか」で決まるので、append-only な履歴と
固定幅で滑る窓は、同じ「history」という語で呼ばれていても**プレフィックスに対して正反対に振る舞う**。
#106 の「剪定できるのは今わかる無関係だけ」と同じく、**入力の時間方向の性質**を見落とすと移植は外れる。
そして **P0 を「コードを書く前に測る」形で置いていたので、実装ゼロで外れが分かった** (#106 と同じ規律が 2 回続けて効いた)。

**次の検証**: 無し (キャッシュを理由にした spec 33 は畳む)。構造の整理だけで進めるなら D6 の振る舞い対照
(継続性・繰り返し・過去の過強調) が先。Meta の「読めるまでの遅延」の所要時間は、必要になったら同一リクエストを
間隔を振って再送すれば測れる。
